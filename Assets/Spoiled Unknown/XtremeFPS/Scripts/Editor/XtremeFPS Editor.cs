...
namespace XtremeFPS.Editor
{
    ...
    public class XtremeFPSEditor : EditorWindow
    {
        ...
        private ChickenController playerArmature;
        ...
        private void OnGUI()
        {
            ...
            if (enablePlayerSetupPanel)
            {
                ...
                playerArmature = (ChickenController)EditorGUILayout.ObjectField(new GUIContent("Player Armature", "The referrence to the player armature gameObject (Leave empty if none exists already)."), playerArmature, typeof(ChickenController), true);
        ...
        private void CreateThePlayer()
        {
            ...
            if (playerArmature == null ||
                !playerArmature.TryGetComponent<ChickenController>(out _))
            {
                GameObject player = GameObject.Find("Player Armature") ?? new GameObject("Player Armature");
                if (!player.TryGetComponent<ChickenController>(out _)) player.AddComponent<ChickenController>();
                playerArmature = player.GetComponent<ChickenController>();
            }
        ...
        private void SetGun()
        {
            weaponObject.chickenController = playerArmature;
